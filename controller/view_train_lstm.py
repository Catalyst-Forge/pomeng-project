...
train_lstm = Blueprint(
    "train_lstm",
    __name__,
    template_folder="templates",
    url_prefix="/dashboard/train-model/lstm",
)
    ...
    try:
        ...
        with open(metadata_path, "r") as f:
            ...
            return {
                "created_at": data.get(
                    "training_date",
                    datetime.fromtimestamp(metadata_path.stat().st_ctime).strftime(
                        "%Y-%m-%dT%H:%M:%S.%f"
                    ),
                ),
                "duration": duration,
...
def list_models() -> List[Dict]:
    ...
    try:
        ...
        for model_dir in model_dirs:
            ...
            if metadata:
                model_info = {"name": model_dir.name, "status": "completed", **metadata}
...
# GET MODEL / index page for LSTM model
...
def get_models():
    try:
        ...
        return render_template(
            "pages/train-model/lstm/index-train-lstm.html", models=models
        )
    except Exception as e:
        ...
        return (
            render_template("pages/train-model/lstm/index-train-lstm.html", models=[]),
            500,
        )


# Handler for select LSTM model
@train_lstm.route("/select_model", methods=["POST"])
...
# Get detail of LSTM model
@train_lstm.route("/<model_name>/details", methods=["GET"])
def get_models_details(model_name):
    ...
    try:
        ...
        return render_template(
            "pages/train-model/lstm/training-lstm-details.html",
...
# GET MODEL METRICS
@train_lstm.route("/<model_name>/metrics", methods=["GET"])
def get_model_metrics(model_name):
    ...
    try:
        ...
        return render_template(
            "pages/train-model/lstm/metrics.html",
            model_name=model_name,
            epochs=epochs,
    ...
    try:
            ...
            layers = [
                {
                    ...
                    "params": {
                        "units": data.get(
                            "lstm_units",
                            Config.DEFAULT_PARAMS["layers"][0]["params"]["units"],
                        ),
                        "dropout": data.get(
                            "lstm_dropout",
                            Config.DEFAULT_PARAMS["layers"][0]["params"]["dropout"],
                        ),
                        "recurrent_dropout": data.get(
                            "lstm_recurrent_dropout",
                            Config.DEFAULT_PARAMS["layers"][0]["params"][
                                "recurrent_dropout"
                            ],
                        ),
                ...
                layers.append(
                    {
                        ...
                        "params": {
                            "units": data.get(
                                "dense_units",
                                Config.DEFAULT_PARAMS["layers"][2]["params"]["units"],
                            ),
                            "activation": data.get(
                                "dense_activation",
                                Config.DEFAULT_PARAMS["layers"][2]["params"][
                                    "activation"
                                ],
                            ),
                        },
                    ...
                    layers.append(
                        {
                            ...
                            "params": {
                                "rate": data.get(
                                    "dense_dropout",
                                    Config.DEFAULT_PARAMS["layers"][3]["params"][
                                        "rate"
                                    ],
                                )
                            },
        ...
        optimizer_config = {
            ...
            "params": {
                "learning_rate": data.get(
                    "learning_rate", Config.OPTIMIZER_CONFIGS["adam"]["learning_rate"]
                )
            },
        }
        ...
        if optimizer_config["name"] in optimizer_mappings:
            optimizer_config["params"].update(
                {
                    param: data.get(
                        param, Config.OPTIMIZER_CONFIGS[optimizer_config["name"]][param]
                    )
        ...
        config = {
            ...
            "embedding_dim": data.get(
                "embedding_dim", Config.DEFAULT_PARAMS["embedding_dim"]
            ),
            "embedding_dropout": data.get(
                "embedding_dropout", Config.DEFAULT_PARAMS["embedding_dropout"]
            ),
        ...
        X_train, Y_train, vocab_size, num_classes, sequence_length = preprocess_data(
            version_path, model_name
        )

        # Log shapes for debugging
        logger.info(f"Training shapes - X: {X_train.shape}, Y: {Y_train.shape}")
        logger.info(
            f"Model parameters - vocab_size: {vocab_size}, num_classes: {num_classes}, sequence_length: {sequence_length}"
        )
...
@train_lstm.route("/<model_name>/delete", methods=["POST"])
