...
@lstm_train.route("/dashboard/training-models/training-lstm/versions", methods=["GET"])
def get_versions():
    ...
    try:
        ...
        def generate_training_progress():
            ...
            class DetailedTrainingCallback(tf.keras.callbacks.Callback):
                ...
                def on_batch_end(self, batch, logs=None):
                    if logs:
                        ...
                        current_step = (
                            (self.current_epoch_data["epoch_number"] - 1)
                            * steps_per_epoch
                            + batch
                            + 1
                        )
                        ...
                        self.sse_queue.put(
                            {"progress": progress, "step_data": step_data}
                        )

                def on_epoch_end(self, epoch, logs=None):
                    end_time = datetime.datetime.now()
                    start_time = datetime.datetime.fromisoformat(
                        self.current_epoch_data["start_time"]
                    )
                    ...
                    training_start = datetime.datetime.fromisoformat(
                        training_history["start_time"]
                    )
            ...
            def stream_response():
                ...
                training_start_time = datetime.datetime.fromisoformat(
                    training_history["start_time"]
                )
                ...
                # Find best accuracy
                best_accuracy = max(
                    epoch["metrics"]["accuracy"] for epoch in training_history["epochs"]
                )
                best_epoch = next(
                    i + 1
...
def get_models():
    ...
    try:
        response = {"models": [], "active_model": None}
                ...
                if metadata_file.exists() and model_file.exists():
                    ...
                    if training_metadata.exists():
                        with open(training_metadata) as f:
                            ...
                            training_metrics = {
                                "final_accuracy": training_history.get(
                                    "final_accuracy"
                                ),
                                "final_loss": training_history.get("final_loss"),
                                "training_duration": training_history.get(
                                    "training_duration"
                                ),
                                "best_accuracy": training_history.get("best_accuracy"),
                                "best_epoch": training_history.get("best_epoch"),
