...
from utils.finetuning.utils import update_selected_model, validate_model_id, get_checkpoints
import logging
...
def list_finetunings():
    ...
    try:
        ...
        list_fine_tuned = []
        current_time = datetime.datetime.now().timestamp()  # Waktu sekarang dalam Unix timestamp
        ...
        for finetuning in response.data:
            ...
            finetuning_data = {
                ...
                "data": {
                    ...
                    "created_at": datetime.datetime.fromtimestamp(finetuning.created_at).strftime('%Y-%m-%d %H:%M:%S') if finetuning.created_at else "N/A",
                    "estimated_finish": datetime.datetime.fromtimestamp(finetuning.estimated_finish).strftime('%Y-%m-%d %H:%M:%S') if finetuning.estimated_finish else "N/A",
                    "fine_tuned_model": finetuning.fine_tuned_model if finetuning.fine_tuned_model else "N/A",
                    "status": finetuning.status if finetuning.status else "N/A",
                    "trained_tokens": finetuning.trained_tokens if finetuning.trained_tokens else "N/A",
                    "result_files": finetuning.result_files if finetuning.result_files else "N/A",
                    "progress": progress
                }
            }
            list_fine_tuned.append(finetuning_data)

        return render_template('pages/training_finetuning/list_finetunings.html', 
...
def finetuning_details(fine_tuning_id):
    try:
        ...
        finetuning_data = {
            ...
            "created_at": datetime.datetime.fromtimestamp(response.created_at).strftime('%Y-%m-%d %H:%M:%S') if response.created_at else "N/A",
            "finished_at": datetime.datetime.fromtimestamp(response.finished_at).strftime('%Y-%m-%d %H:%M:%S') if response.finished_at else "N/A",
            "fine_tuned_model": response.fine_tuned_model if response.fine_tuned_model else "N/A",
            "organization_id": response.organization_id if response.organization_id else "N/A",
            "result_files": response.result_files if response.result_files else "N/A",
            "status": response.status if response.status else "N/A",
            "validation_file": response.validation_file if response.validation_file else "N/A",
            "training_file": response.training_file if response.training_file else "N/A",
            "hyperparameters": {
                "n_epochs": response.hyperparameters.n_epochs if response.hyperparameters else "N/A",
                "batch_size": response.hyperparameters.batch_size if response.hyperparameters else "N/A",
                "learning_rate_multiplier": response.hyperparameters.learning_rate_multiplier if response.hyperparameters else "N/A"
            },
            "trained_tokens": response.trained_tokens if response.trained_tokens else "N/A",
            "integrations": response.integrations if response.integrations else "N/A",
            "seed": response.seed if response.seed else "N/A",
            "estimated_finish": response.estimated_finish if response.estimated_finish else "N/A",
            "checkpoints": checkpoints
        }
        return render_template('pages/training_finetuning/modal_finetuning.html', finetuning=finetuning_data)
    except Exception as e:
...
def show_metrics(fine_tuning_id: str) -> str:
    ...
    try:
        ...
        if job.result_files:
            ...
            logger.warning(f"No result files found for fine-tuning job: {fine_tuning_id}")
        
        logger.info(f"Rendering template with metrics data available: {bool(metrics_data)}")
        
        return render_template(
            'pages/training_finetuning/chart.html',
    ...
    try:
        ...
        if metrics_data and 'step' in metrics_data and metrics_data['step']:
            ...
            paginated_metrics = {
                ...
                'step': metrics_data['step'][start_idx:end_idx] if metrics_data.get('step') else [],
                'train_loss': metrics_data['train_loss'][start_idx:end_idx] if metrics_data.get('train_loss') else [],
                'train_accuracy': metrics_data['train_accuracy'][start_idx:end_idx] if metrics_data.get('train_accuracy') else [],
                'valid_loss': metrics_data['valid_loss'][start_idx:end_idx] if metrics_data.get('valid_loss') else [],
                'valid_mean_token_accuracy': metrics_data['valid_mean_token_accuracy'][start_idx:end_idx] if metrics_data.get('valid_mean_token_accuracy') else []
            }
        else:
            paginated_metrics = {
                'avg_train_loss': None,
        ...
        logger.info(f"Rendering template with metrics data available: {bool(paginated_metrics)}")
        
        return render_template(
            'pages/training_finetuning/modal_steps.html',
            fine_tuning_id=fine_tuning_id,
    ...
    try:
        ...
        try:
            ...
            training_file = client.files.create(
                file=open("dataset/finetuning.jsonl", "rb"),
                purpose="fine-tune"
            )
